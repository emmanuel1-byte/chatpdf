...
from .schema import SignupSchema, LoginSchema, VerifyOTPSchema, ResendVerificationEmailSchema
...
async def resend_verification_email(
    ...
    if user.verified:
        raise HTTPException(status_code=409, detail={"message":"Account already verified"})

    otp_data = generate_otp()
    background_tasks.add_task(
        send_verification_email,
        user.model_dump(mode="json").get("email"),
        validated_request.fullname.split(" ")[0],
        otp_data.get("password"),
    )
...
async def login(
    ...
    compare_password = bcrypt.checkpw(
        ...
        user.model_dump(mode="json").get("password").encode(),
    )
    if not compare_password:
        raise HTTPException(status_code=401, detail={"message": "Invalid credentials"})

    access_token, refresh_token = generate_tokens(
        user.model_dump(mode="json").get("id")
    )
